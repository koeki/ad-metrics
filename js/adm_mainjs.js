...
var tag_config = {
					generic: { parameters: ["ii","cn","dfp","ev","sz","mve","mhe","uum","cl","ca","pl","cr","tp","ts"], value: "generic" },
					rovermediaplex: { parameters: ["dfp","ev","mve","mhe","uum","ca","tp","ts"], value: "rovermediaplex" , unimportant : ["ii","cn","sz","cl","pl","cr"]},
					mediamind: {parameters: ["dfp","ev","mve","mhe","uum","ca","tp","ts"], value: "mediamind" ,unimportant : ["ii","cn","sz","cl","pl","cr"]}
			
				};
var parameterconfig = { dfp:{type: "radio",},
						ev: {type: "check",},
						sz: {type: "dropdown",},
						mve:{type: "dropdown",},
						mhe:{type: "dropdown",},
...
// sets value of exunneded inputs to ""
			for (var i = 0 ; i < b.length ; i++) {
				if (b[i] != null && b[i] != "" && b[i] != "dfp" && b[i] != "ev" && document.getElementById(b[i]).value == "not needed"){
				...
				for (property in tag_config[tagtype]){
					...
					if (tagtype == radiovalue) {
						...
						if (property == "unimportant") {
							...
							do {
								if (unimp[i] == "ii" || unimp[i] =="cn" || unimp[i] =="sz" || unimp[i] =="cl"|| unimp[i] =="pl"|| unimp[i] =="cr")
...
//output inhalt "texarea" 
		 	if(b.length != 0 ){
				...
				for (var i = 0  ; i < b.length ; i++ ) {
					 ...
					 if (formelement.checked  && formelement != "param_dfp" && formelement != "param_ev" && b[i] != "dfp" && b[i] != "ev" ) {
					 	if (document.getElementById(b[i]).value != null && document.getElementById(b[i]).value != b[i]+ " value" && document.getElementById(b[i]).value != ""){
...
//device fingerprint 
					 if (formelement.checked && b[i] == "dfp" ) {
					 	...
					 	var inhaltdfp ="";
					 		...
					 		if (document.tagp.track.value == "true") {
					 			inhaltdfp = "&dfp=1";
					 	
					 		} else if (document.tagp.track.value == "false"){
					 			inhaltdfp = "&dfp=0"
					 		}
...
//event config 					 
					 ...
					 if (formelement.checked && b[i]=="ev")	{
						 ...
						 for (var eva = 0; eva < evw.length ; eva++) {

						 	formelem = document.getElementById("evvalue_" + evw[eva]);
						 	

						 	if (formelem.checked){
						 		
						 		evvalue_step1 = "&ev=";
						 		evvalue_step2 += evw[eva];
						 		evvaluegesamt = evvalue_step1 + evvalue_step2
...
// sets value of drop down textaias if the value isnt default or own value 
			...
			if (b.length != 0 ){
				...
				for (var i = 0  ; i < b.length ; i++ ) {
					 ...
					 formelement = document.getElementById("param_"+b[i]);
					 	...
					 	if ( b[i] == "sz" || b[i] == "mve"|| b[i] == "mhe"|| b[i] == "cn"|| b[i] == "uum"|| b[i] == "cl"|| b[i] == "ca"){

					 		var ddinhalt  =  document.getElementById(b[i] + "dd").value;

					 		if (ddinhalt == "default"){
					 			defaultvalue();
					 		} else if (ddinhalt == "own value"){
					 			document.getElementById(b[i]).value = "";
					 		}else{
...
// sets drop down texarea value if ists default
		function defaultvalue() {
			for (var i = 0;i < b.length;i++){
						if (b[i]=="sz" || b[i]=="mve" || b[i]=="mhe" || b[i]=="cn" || b[i]=="cl" || b[i]=="ca"){
							var ddinputvalue = document.getElementById(b[i]+"dd").value;
							if(ddinputvalue == "default") {
								switch (b[i]) {
									case "sz":  document.getElementById("sz").value = "0x0";
									case "mve": document.getElementById("mve").value = "unlimited";
									case "mhe": document.getElementById("mhe").value = "unlimited";
									case "cn": document.getElementById("cn").value = "0";
									case "cl": document.getElementById("cl").value = "_";
									case "ca": document.getElementById("ca").value = "_";
								}
							}
						}
			}
